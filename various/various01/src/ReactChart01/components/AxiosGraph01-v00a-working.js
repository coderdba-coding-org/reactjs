...
export default class AxiosGraph01 extends React.Component {
    ...
    axios.get(`https://jsonplaceholder.typicode.com/users`)
      .then(res => {
        ...
        pNames = this.state.persons.map((item) =>
            ...
            //qNames = qNames.concat(item.name)
            //qNames.concat(item.name)
  ...
  render() {
      ...
      // this works, if the state property is set right
      const labels = this.state.personNames

      // THIS ALSO WORKS --> but, create a looping function to do this concatentation
      /*
      labels = []
      labels = labels.concat("1")
      labels = labels.concat("2")
      labels = labels.concat("3")
      */
      ...
      */
      ...
      /* THIS WORKS - this is convoluted, not sure why this way --> but, create a looping function to do this concatentation
      var labels = []
      console.log(labels)
